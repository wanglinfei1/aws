...
function addMongo(obj) {
    function savStock(){
        obj.id=UUID();
        obj.time=new Date();
        dbHandler('add',tabName,obj).then((data) => {
            console.log(obj.movieName+'   添加成功')
        })
    }
    function updata() {
        obj.time=new Date();
        dbHandler('update',tabName,[{movieName: obj.movieName},{$set:obj}]).then((data)=> {
            console.log(obj.movieName+'   更新成功')
        });
    }
    dbHandler('find',tabName,{"movieName":obj.movieName}).then((data) =>{
        if(data.length){
            updata(data)
        }else{
            savStock();
